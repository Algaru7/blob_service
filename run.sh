...
docker run -ti -d --name blob --hostname blob -p 3002:3002 -v $PWD/persistence/blob:/src/persistence-blob ${base_repo}/servicio-adi:blob
docker run -ti -d --name auth --hostname auth -p 3001:3001 -v $PWD/persistence/auth:/src/persistence-auth ${base_repo}/servicio-adi:auth
docker run -ti -d --name dir  --hostname dir  -p 3003:3003 -v $PWD/persistence/dir:/src/persistence-dir ${base_repo}/servicio-adi:dir
